...
from lib.ataques.rsa_franklin_reiter.util_rsa import generate_rsa_keypair as gerador_rsa_chaves, rsa_encrypt as rsa_encriptador
from crypto_io.util_io import texto_para_inteiro
import secrets


def gerar_chaves(bits: int):
    if bits < 256:
...
def gerar_mensagens_relacionadas(n: int, a: Optional[int], b: Optional[int], texto: Optional[str]) -> Tuple[int, int, int, int, Optional[int]]:
    """
    Retorna (m1, m2, a, b, nbytes).
    nbytes é usado apenas se texto != None.
    """
...
def gerar_caso_textos(texto1, texto2, bits=1024, a=None, b=None, encrypt_func=None):
    n, e, d = gerar_chaves(bits)
...
def gerar_caso_inteiros(m1, m2, bits=1024, a=None, b=None, encrypt_func=None):
    n, e, d = gerar_chaves(bits)
    m1 = m1 % n
    m2 = m2 % n
    if a is None:
...
def gerar_caso_m1(m1=None, texto1=None, bits=1024, a=None, b=None, encrypt_func=None):
    n, e, d = gerar_chaves(bits)
    nbytes = None
...
def gerar_caso_aleatorio(bits=1024, a=None, b=None, encrypt_func=None):
    n, e, d = gerar_chaves(bits)
...
def gerador_caso_relacionado_linear(bits=1024, a=None, b=None, texto1=None, texto2=None, m1=None, m2=None, encrypt_func=None):
    if texto1 is not None and texto2 is not None:
        return gerar_caso_textos(texto1, texto2, bits, a, b, encrypt_func)
    elif m1 is not None and m2 is not None:
        return gerar_caso_inteiros(m1, m2, bits, a, b, encrypt_func)
    elif texto1 is not None and texto2 is None:
        return gerar_caso_m1(m1=None, texto1=texto1, bits=bits, a=a, b=b, encrypt_func=encrypt_func)
    elif m1 is not None and m2 is None:
        return gerar_caso_m1(m1=m1, texto1=None, bits=bits, a=a, b=b, encrypt_func=encrypt_func)
    else:
        return gerar_caso_aleatorio(bits, a, b, encrypt_func)